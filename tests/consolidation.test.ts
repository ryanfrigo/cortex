...
describe('consolidation', () => {
  ...
  beforeAll(async () => {
    ...
    await engine.save({ content: 'Ryan had oatmeal for breakfast on Monday morning', namespace: 'health' });
    await engine.save({ content: 'Ryan ate oatmeal for breakfast on Monday', namespace: 'health' });
    await engine.save({ content: 'Ryan had oatmeal Monday morning for breakfast', namespace: 'health' });
    // Save a different memory
    await engine.save({ content: 'VoiceCharm uses Next.js and Convex for the backend', namespace: 'projects' });
  });
  ...
  it('does not cluster dissimilar memories', async () => {
    const result = await engine.consolidate({ dryRun: true, similarityThreshold: 0.85, minClusterSize: 2 });
    // VoiceCharm memory should not be in the oatmeal cluster
    for (const cluster of result.clusters) {
      const hasOatmeal = cluster.contents.some(c => c.includes('oatmeal'));
      const hasVoiceCharm = cluster.contents.some(c => c.includes('VoiceCharm'));
      expect(hasOatmeal && hasVoiceCharm).toBe(false);
    }
  });

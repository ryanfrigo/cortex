...
describe('generateTiers', () => {
  it('produces non-empty l0 and l1 from a sentence', () => {
    const { l0, l1 } = generateTiers('User uses TypeScript for all projects.');
...
describe('MemoryEngine — hierarchical namespaces', () => {
  ...
  it('namespacePrefix matches subtrees', async () => {
    await engine.save({ content: 'MyApp project memory alpha', namespace: 'projects/myapp' });
    ...
    expect(namespaces).toContain('projects/myapp');
